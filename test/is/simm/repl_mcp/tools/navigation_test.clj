  ...
  (testing "tools vector exists and has correct structure"
    ...
    (testing "specific tool names"
      (let [tool-names (set (map :name nav-tools/tools))]
        (is (not (contains? tool-names "call-hierarchy")))
...
(deftest usage-format-test
  (testing "usage finder preserves refactor-nrepl line and column positions"
    (let [result (nav-tools/format-usage-analysis
                  {:status :success
                   :value {:namespace "test.ns"
                           :symbol "target"
                           :usages [{:file "/tmp/example.clj"
                                     :line-beg 12
                                     :line-end 12
                                     :col-beg 7
                                     :col-end 13}]}})]
      (is (str/includes? result "/tmp/example.clj (line 12, col 7)"))
      (is (not (str/includes? result "line )"))))))
